...
- **DUAL_DL_TAMAR (TMVE8)**: cotiza en pesos por 100 VN en USD, como los dólar-linked;
  payoff = `max(fx_base × riel TAMAR, 100 × FX)` con FX = mayorista venta vivo y, si no hay,
  el fixing A3500 del settle — **sin proyectar**, sin lag ni spread en el riel DL; reusa
  `tamar_dual_payoff_at` sin tocarlo. Sin `fx_base`, sin ningún FX o sin serie TAMAR →
  `None`: nunca se precia con el riel TAMAR solo. Grupo propio `DUAL_DL` (fuera de BEI y de
  la curva `tamar`). Popup: sólo la pata `_DL` (TIR en USD).
...
- **Rutas públicas = `tests/test_aud_G_tests_route_auth.py::_PUBLIC_PATHS`** (`/login`,
  `/logout`, `/api/health`, `/reset/{token}`, `/forgot`, `/static`). Todo router nuevo lleva
  `RequireTabPermission`; no crear endpoints de salud nuevos; `/api/health` es público y por
  eso recortado.
- **Un link que viaja por mail se arma con `settings.public_url`, nunca con el Host del request**
  (`reset_service.mail_link`; `mail_enabled` exige `MONITOR_PUBLIC_URL`): un POST anónimo a `/forgot`
  elige su `Host` (reset poisoning). Guardián:
