...
**Auditoría UI/UX del 2026-09-09 (18 candidatas, fuentes primarias).** La actividad indicada
es `pushed_at` consultado ese día en la API oficial de GitHub; prueba actividad del repo, no
garantiza mantenimiento futuro. “Local” significa que el runtime no transmite datos si se
sirve el asset vendorizado; descargar por npm/GitHub sí contacta esos servicios. No se
instaló nada. Las reglas resultantes viven únicamente en `docs/ui-ux.md`.

| Proyecto / estándar | Veredicto y problema concreto | Licencia · mantenimiento · permisos/transmisión/costo · reversión |
|---|---|---|
| **[GridStack](https://github.com/gridstack/gridstack.js)** | **MANTENER** el motor actual; **PROBAR** el salto mayor sólo en una fase propia. Ya resuelve layout 2D, resize, colisión y persistencia sin framework. El bundle local declara `GDRev="11.5.1"` en `apps/web/static/vendor/gridstack/gridstack-all.js:2`. Desde 13.1 el upstream puede mantener el DOM en orden visual para tabulación/impresión, pero 11→13 incluye migraciones de CSS/columnas ([changelog](https://github.com/gridstack/gridstack.js/blob/master/doc/CHANGES.md)). | [MIT](https://github.com/gridstack/gridstack.js/blob/master/LICENSE); push 2026-09-04; sin servicio, cuenta, telemetría ni red en runtime local. Reversión de un upgrade: restaurar JS/CSS vendor e init; retirar GridStack por completo exigiría quitar vendor, enlaces e integración/persistencia y volver al fallback apilado. |
| **[Chart.js](https://github.com/chartjs/Chart.js)** | **MANTENER** y **ADOPTAR** su guía de accesibilidad. El bundle local es 4.4.3; reemplazarlo no corrige que hoy haya canvas sin alternativa. [La documentación oficial](https://www.chartjs.org/docs/latest/general/accessibility.html) dice que el contenido canvas no llega al lector de pantalla. | [MIT](https://github.com/chartjs/Chart.js/blob/master/LICENSE.md); push 2026-05-27; local, gratis y sin transmisión. Reversión de un upgrade: restaurar el vendor; retirarlo exige borrar cada init y ofrecer otra representación de todos los gráficos. |
| **[WCAG 2.2](https://www.w3.org/TR/WCAG22/) + [ARIA APG](https://github.com/w3c/aria-practices)** | **ADOPTAR como lectura obligatoria**, en especial dragging, target size, focus, reflow, status, tabla sortable y grid. Proporcionan el contrato que falta; APG es guía ilustrativa, no prueba de compatibilidad. No afirmar conformidad sin evaluación. | [Términos W3C](https://www.w3.org/copyright/software-license/); repos `w3c/wcag` push 2026-09-08 y `w3c/aria-practices` 2026-08-21; lectura pública, gratis, sin acceso al Monitor. Reversión: quitar los enlaces/reglas derivadas; no hay runtime. |
| **[Playwright](https://github.com/microsoft/playwright)** | **CONFIGURAR MEJOR LO NATIVO**: conservar el MCP y `/verificar-ui`; **POSTERGAR** instalar Playwright Test hasta tener auth y datos sintéticos deterministas. `toHaveScreenshot` cubre regresión, pero [el upstream exige el mismo entorno del baseline](https://playwright.dev/docs/test-snapshots). | [Apache-2.0](https://github.com/microsoft/playwright/blob/main/LICENSE); push 2026-09-09; el browser local ve DOM, cookies y pantalla autenticada, por lo que capturas/trazas quedan en scratch. Sin servicio pago; una instalación futura descarga paquetes/browsers. Reversión futura: quitar dependencia, config, browsers y baselines. |
| **[axe-core](https://github.com/dequelabs/axe-core) + [@axe-core/playwright](https://github.com/dequelabs/axe-core-npm/tree/develop/packages/playwright)** | **PROBAR** más adelante con una corrida autenticada local; no instalar ahora. Automatiza fallos detectables, pero el propio proyecto estima 57 % de problemas WCAG y [Playwright exige combinarlo con pruebas manuales](https://playwright.dev/docs/accessibility-testing). | [MPL-2.0](https://github.com/dequelabs/axe-core/blob/develop/LICENSE); pushes 2026-09-08/02; analiza el DOM local sin servicio ni cuenta. Productos/servicios Deque son separados y no hacen falta. Reversión: quitar paquete y tests; toda exclusión futura debe tener issue, motivo y vencimiento. |
| **[Anthropic frontend-design](https://github.com/anthropics/skills/tree/main/skills/frontend-design)** | **ADOPTAR como lectura breve, no instalar**. Sirve para empezar por propósito, identidad existente, responsive, foco y movimiento; sus gustos genéricos quedan subordinados al Monitor financiero y a `docs/ui-ux.md`. | [Apache-2.0](https://github.com/anthropics/skills/blob/main/skills/frontend-design/LICENSE.txt); repo push 2026-09-03; leer envía sólo la instrucción al contexto del agente, sin runtime ni servicio adicional. Reversión: quitar el enlace/extracto. |
| **[Impeccable](https://github.com/pbakaus/impeccable)** | **POSTERGAR** el detector; rescatar sólo su checklist. Tiene 61 reglas deterministas sin LLM/API key, pero instalar agrega skill, launcher/binario, hooks y genera `PRODUCT.md`/`DESIGN.md`, fuentes paralelas prohibidas por §0.1.4. Su modo live puede ejecutar un script del proyecto con permisos del usuario. | [Apache-2.0](https://github.com/pbakaus/impeccable/blob/main/LICENSE); push 2026-09-09; gratis, pero npm/GitHub descargan el binario y el modo browser inspecciona el DOM. Reversión: quitar skill, hooks, binario `~/.impeccable/bin` y archivos generados. |
| **[DTCG Design Tokens 2025.10](https://www.designtokens.org/TR/2025.10/format/)** | **POSTERGAR** el formato y **ADOPTAR sólo vocabulario semántico**. Es intercambio entre herramientas; para un único destino CSS agregaría otra fuente/build. Es Final Community Group Report estable, no Recomendación W3C; el [draft](https://www.designtokens.org/tr/drafts/format/) dice expresamente que no se implemente. | W3C Community Final Specification Agreement, no licencia SPDX de software; repo push 2026-09-08; lectura pública, sin permisos ni runtime. Reversión: quitar el enlace; no crear token files. |
| **[SortableJS](https://github.com/SortableJS/Sortable)** | **DESCARTAR**. Reordena listas; no aporta resize ni colisión 2D y duplicaría GridStack. La clase local `.uni-sortable` es sorting vanilla, no esta dependencia. | [MIT](https://github.com/SortableJS/Sortable/blob/master/LICENSE); push 2026-03-24; local y gratis. Reversión si alguna vez se prueba: quitar vendor/paquete e init. |
| **[interact.js](https://github.com/taye/interact.js)** | **DESCARTAR**. Es una primitiva drag/resize/snap; obligaría a reconstruir colisión, persistencia, responsive y accesibilidad ya cubiertos por GridStack. | [MIT](https://github.com/taye/interact.js/blob/main/LICENSE); push 2026-08-01; local y gratis. Reversión: quitar paquete/vendor, listeners y transforms. |
| **[BackstopJS](https://github.com/garris/BackstopJS)** | **DESCARTAR**. Duplica screenshots de Playwright, trae Playwright/Puppeteer/Docker y su README pide nuevo mantenedor. Su default `ignoreHTTPSErrors: true` contradice el guard TLS del repo. | [MIT](https://github.com/garris/BackstopJS/blob/master/LICENSE); push 2026-09-08 pese al pedido de mantenedor; puede leer URLs, cookies y `storageState`, y Docker descarga imagen. Reversión: quitar paquete, `backstop.json` y `backstop_data/`. |
| **[Storybook](https://github.com/storybookjs/storybook)** | **POSTERGAR**. Aunque soporta HTML, exige Node 20+, servidor/build y stories que duplicarían estados Jinja; [el instalador crea boilerplate y activa telemetría](https://storybook.js.org/docs/get-started/install). Sólo reconsiderar ante una biblioteca de componentes independiente. | [MIT](https://github.com/storybookjs/storybook/blob/next/LICENSE); push 2026-09-09; core local/gratis. Chromatic es servicio cloud comercial separado y recibe builds/capturas. Reversión: quitar paquetes, scripts, `.storybook/` y stories. |
| **[Style Dictionary](https://github.com/style-dictionary/style-dictionary)** | **POSTERGAR**. Está hecho para generar estilos de una fuente a varias plataformas; aquí sólo generaría CSS y exige Node 22. Variables CSS existentes son la opción más simple. | [Apache-2.0](https://github.com/style-dictionary/style-dictionary/blob/main/LICENSE); push 2026-09-09; CLI local, gratis; npm sólo al instalar. Reversión: quitar dependencia, config, tokens y generados. |
| **[Open Props](https://github.com/argyleink/open-props)** | **DESCARTAR**. Su catálogo genérico crearía una paleta/escala competidora y drift respecto de los tokens financieros actuales. | [MIT](https://github.com/argyleink/open-props/blob/main/LICENSE); push 2026-08-11; gratis/local si se vendoriza; CDN transmitiría URLs/IP y agrega dependencia de red. Reversión: quitar import/vendor. |
| **[Tabulator](https://github.com/tabulator-tables/tabulator)** | **POSTERGAR**. Convierte HTML/arrays/JSON en una grilla que pasa a poseer el DOM, complicando swaps HTMX/SSE. Reconsiderar sólo ante una medición de miles de filas que exija virtualización. | [MIT](https://github.com/tabulator-tables/tabulator/blob/master/LICENSE); push 2026-09-09; local y gratis, sin servicio. Reversión: quitar CSS/JS/init y restaurar tabla semántica SSR. |
| **[AG Grid](https://github.com/ag-grid/ag-grid)** | **DESCARTAR**. Soporta JS plano, pero reemplaza el camino SSR por una grilla mucho mayor; pivot, server-side model, charts y soporte avanzados son Enterprise. No agregar su MCP. | [Community MIT; Enterprise comercial](https://github.com/ag-grid/ag-grid/blob/latest/LICENSE.txt); push 2026-09-09; runtime local, licencia/costo para Enterprise. Reversión: quitar paquetes/assets/init y cualquier license key. |
| **[Vega-Lite](https://github.com/vega/vega-lite)** | **POSTERGAR**. Aporta specs declarativas, pero [el embed normal requiere Vega + Vega-Lite + Vega-Embed](https://vega.github.io/vega-lite/usage/embed.html). No hay hueco medido que justifique reemplazar Chart.js. | [BSD-3-Clause](https://github.com/vega/vega-lite/blob/main/LICENSE); push 2026-09-08; local si se vendorizan tres assets; CDN/editor online añaden red. Reversión: quitar assets, specs y loader. |
| **[UI UX Pro Max](https://github.com/nextlevelbuilder/ui-ux-pro-max-skill)** | **DESCARTAR como instalación** por §0.4/§0.9. Es un catálogo grande de estilos, paletas, stacks y patrones con CLI/autoactivación; agrega contexto genérico y duplica Superpowers. Sólo se rescata su checklist de foco, contraste, reflow y reduced motion. | [MIT para la edición básica](https://github.com/nextlevelbuilder/ui-ux-pro-max-skill/blob/main/LICENSE), oferta Premium comercial separada; push 2026-09-06; instalador contacta npm/marketplace y escribe skills. Reversión declarada: `uipro uninstall`; retirar también cualquier config generada. |
