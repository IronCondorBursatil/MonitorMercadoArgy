---
name: compound
description: Checklist de cierre de fase/feature/bugfix antes de pedir merge (agents.md §0.1.9 y §0.8) — qué lección quedó materializada (test guardián, hook/permiso, línea de CLAUDE.md/agents.md), qué memoria y qué doc se corrigieron, qué quedó fuera de alcance, y el bloque de reporte para el PR.
...
| **Hook / permiso** | la lección es "no hacer X" | la regla exacta en `.claude/settings.json` (`allow`/`ask`/`deny`) o el hook, y la llamada deliberada con la que se probó que bloquea |
| **Línea de `CLAUDE.md`** | es un invariante del repo (no romper) | sección "Invariantes" o "Robustez"; una línea, sin narrativa histórica |
...
`~/.claude/projects/c--Users-david-OneDrive-Monitores---Data912/memory/` + la línea del
...
herramientas → `agents.md §0`; invariantes → `CLAUDE.md`; convenciones financieras →
...
Destinos: sección "Pendientes" del PR · `CLAUDE.md` "Pendiente (cola, no funcional)" si es
...
- Materializada en: <test id + mutación probada | regla/hook | CLAUDE.md §… | agents.md §0.x | docstring>
